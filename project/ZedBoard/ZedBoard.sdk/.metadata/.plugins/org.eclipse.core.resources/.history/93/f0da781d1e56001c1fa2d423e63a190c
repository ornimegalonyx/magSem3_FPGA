...
void vga_frame_draw_rectangle_filled(vga_frame* vga_frame_obj,int x_0,int y_0,int width,
		int height,vga_pixel color, bool filled) {
	int x,y;
	int x_new,y_new;
	for(y=0; y<=height; y++) {
		for(x=0; x<=width; x++) {
			...
			if (vga_is_within_borders_x(x_new)&&
				...
				if (filled)
					vga_frame_get_pixel(vga_frame_obj)[y_new][x_new] = color;
						...
						(width - x) <= BORDER_WIDTH || (height - y) <= BORDER_WIDTH)
					vga_frame_get_pixel(vga_frame_obj)[y_new][x_new] = color;
			}
		}
	}
...
void vga_frame_draw_line(int16_t x1, int16_t y1, int16_t x2, int16_t y2){
	if (y1 == y2 || x1 == x2)
		return;
	int16_t yi = y1, xi = x1;
	int16_t temp1 = abs(y2 - y1);
	int16_t temp2 = abs(x2 - x1);
	if (temp1 > temp2){
		for (yi = y1; yi != y2; yi += (y2 > y1) ? 1 : -1){
			xi = (yi - y1)*(x2 - x1)/(y2 - y1) + x1;
			vga_frame_set_pixel(xi, yi);
		}
	}
	else {
		for (xi = x1; xi != x2; xi += (x2 > x1) ? 1 : -1){
...
void vga_frame_draw_rect(bool filled, int16_t x1, int16_t y1, int16_t x2, int16_t y2){
	vga_frame_draw_rectangle_filled(&our_vga_frame, x1, y1, abs(x2 - x1), abs(y2 - y1), cur_color, filled);
}

void vga_frame_draw_ellipse(bool filled, int16_t x1, int16_t y1, int16_t x2, int16_t y2){
	if (y1 == y2 || x1 == x2)
		return;
	int x,y;
	...
	for(y=min(y1, y2); y<=(max(y1, y2)); y++) {
	    for(x=min(x1, x2); x<=(max(x1,x2)); x++) {
	        if (vga_is_within_borders_x(x) &&
	            vga_is_within_borders_y(y))
	        	if (((x - x0) * 1.0 * (x - x0) / (a * a) + (y - y0) * 1.0 * (y - y0) / (b*b)) < 1)
	        		if (filled)
	        			vga_frame_get_pixel(&our_vga_frame)[y][x] = cur_color;
	        		else if(((x - x0) * 1.0 * (x - x0) / ((a-BORDER_WIDTH) * (a-BORDER_WIDTH)) +
	        				 (y - y0) * 1.0 * (y - y0) / ((b-BORDER_WIDTH) * (b-BORDER_WIDTH))) > 1)
	        			vga_frame_get_pixel(&our_vga_frame)[y][x] = cur_color;
	    }
	}
}
...
void gpioRW() {
	...
	if ((sw >> 7) & 0x1) {
		...
		case 0x01:
			curColor = clrBlack;
			cur_color = {(0x0), (0x0), (0x0)}; //clrBlack;
			break;
		case 0x02:
			curColor = clrMedGray;
			cur_color = {(0x8), (0x8), (0x8)}; //clrMedGray;
			break;
		case 0x04:
			curColor = clrMagenta;
			cur_color = {(0xf), (0x0), (0xf)}; //clrMagenta;
			break;
		case 0x08:
			curColor = clrBlue;
			cur_color = {(0x0), (0x0), (0xf)}; //clrBlue;
			break;
		case 0x10:
			curColor = clrGreen;
			cur_color = {(0x0), (0xf), (0x0)}; //clrGreen;
			break;
		case 0x20:
			curColor = clrYellow;
			cur_color = {(0xf), (0xf), (0x0)}; //clrYellow;
			break;
		case 0x40:
			curColor = clrRed;
			cur_color = {(0xf), (0x0), (0x0)}; //clrRed;
			break;
		case 0x80:
			curColor = clrWhite;
			cur_color = {(0xf), (0xf), (0xf)}; //clrWhite;
			break;
		default:
			break;
		}
...
void paint() {
	...
	if ((sw & 0xF) == 1) {
		...
		vga_frame_set_pixel(rgfng[0].x, rgfng[0].y);
	}
	if (rgfng[0].st == FINGER_UP) {
		...
		case 2:
			...
			vga_frame_draw_line(lastDown.x, lastDown.y, lastMove.x, lastMove.y);
			break;
		case 4:
			mydisp.drawRectangle(curFill,
					min(lastMove.x, lastDown.x), min(lastMove.y, lastDown.y),
					max(lastMove.x, lastDown.x), max(lastMove.y, lastDown.y));
			vga_frame_draw_rect(curFill,
					min(lastMove.x, lastDown.x), min(lastMove.y, lastDown.y),
					max(lastMove.x, lastDown.x), max(lastMove.y, lastDown.y));
			break;
		case 8:
			mydisp.drawEllipse(curFill,
					min(lastMove.x, lastDown.x), min(lastMove.y, lastDown.y),
					max(lastMove.x, lastDown.x), max(lastMove.y, lastDown.y));
			vga_frame_draw_ellipse(curFill,
					min(lastMove.x, lastDown.x), min(lastMove.y, lastDown.y),
					max(lastMove.x, lastDown.x), max(lastMove.y, lastDown.y));
			break;
