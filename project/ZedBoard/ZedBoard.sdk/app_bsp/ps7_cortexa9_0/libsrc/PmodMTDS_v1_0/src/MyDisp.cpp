...
bool MYDISP::drawArc(int x1, int y1, int x2, int y2, int xr1, int yr1, int xr2,
		int yr2) {

	return mtds.Arc(hdsDisp, (int16_t) x1, (int16_t) y1, (int16_t) x2,
			(int16_t) y2, (int16_t) xr1, (int16_t) yr1, (int16_t) xr2,
			(int16_t) yr2);
...
bool MYDISP::drawRectangle(bool fill, int x1, int y1, int x2, int y2) {
	...
	return mtds.Rectangle(hdsDisp, (int16_t) x1, (int16_t) y1, (int16_t) x2,
			(int16_t) y2);
...
bool MYDISP::drawEllipse(bool fill, int x1, int y1, int x2, int y2) {
	...
	return mtds.Ellipse(hdsDisp, (int16_t) x1, (int16_t) y1, (int16_t) x2,
			(int16_t) y2);
...
bool MYDISP::createButton(int id, HBMP hbmp1, HBMP hbmp2, int x, int y) {
	 ...
	 ** can erase it later.
	 */
	if ((rgbtn[id].hbmpBg = mtds.CreateBitmap(bmpd.dxco, bmpd.dyco, bmpd.cbpp))
			== 0) {
		goto lErrorExit;
	}
	...
	/* Error occured. Clean up and get out.
	 */
	lErrorExit: DeleteBtnBitmaps(id);
	return false;
}
	...
	case BUTTON_UP:
		...
		if ((rgbtn[id].fs & fsBtnImageUp) == 0) {
			...
			mtds.BitBlt(hdsDisp, rgbtn[id].rct.xcoLeft, rgbtn[id].rct.ycoTop,
					rgbtn[id].dxco, rgbtn[id].dyco, hdsBlt, 0, 0, ropSrcCopy);
			...
			mtds.DrawBitmap(hdsDisp, rgbtn[id].rct.xcoLeft,
					rgbtn[id].rct.ycoTop, rgbtn[id].dxco, rgbtn[id].dyco,
					hdsBlt, 0, 0);
			mtds.SetIntensity(hdsDisp, ityCur);
		}
...
void MYDISP::checkTouch() {
	...
	if (idBtnTrack == -1) {
		...
		if (mtch.msg == msgFinger1Move) {
			if (!mtds.PointInRect(&(rgbtn[idBtnTrack].rct), mtch.xco,
					mtch.yco)) {
...
int MYDISP::IdBtnFromXcoYco(int16_t xco, int16_t yco) {
	...
	for (ibtn = 0; ibtn < NUM_BUTTONS; ibtn++) {
		if (((rgbtn[ibtn].fs & fsBtnAlloc) != 0)
				&& ((rgbtn[ibtn].fs & fsBtnEnabled) != 0)) {
